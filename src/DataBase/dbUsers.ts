...
class UsersDb {
  ...
  async addNew(user: string): Promise<ResponseInterface> {
    try {
      ...
      if (!fixedUserWithParse) {
        return {
          msg: "User is required user must be JSON or urlencoded",
          status: 400,
        };
      }
      const userBody = {
  ...
  async update(bodyPeace: any, id: string): Promise<ResponseInterface> {
    try {
      ...
      let index = 0;
      const user: UserInterface | undefined = userDB.find(
        (user: UserInterface, idx: number) => {
          index = idx;
          return user.id === id;
        }
      );
      ...
      const required: ResponseInterface[] = validateBody(bodyPeace);
